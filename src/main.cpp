...
// int32_t handle_request(int conn_fd) {
//     /*
//         protocol spec
...
//     // Reading first 4 bytes to get msg len
//     int32_t err = read_full(conn_fd, rbuf, 4);

//     if (err) {
//         std::cerr << (errno == 0 ? "EOF while reading msg_size from conn"
...
//     if (err) {
//         std::cerr << (errno == 0 ? "EOF while reading msg from conn"
//                                  : "Error reading message from conn")
//                   << std::endl;
//         return err;
//     }

//     std::cout << "Client message recieved: " << std::string(rbuf + 4, len)
//               << std::endl;
...
int main(void) {
    ...
    //     int conn_fd = accept(s_fd, (struct sockaddr *)&client_addr,
    //     &addrlen); if (conn_fd == -1) {
    //         // error
    //         std::cerr << "Unable to connect to this client..." << std::endl;
    //         continue;
    //     }
